...
function blrt_gallery($atts){
    ...
    wp_enqueue_script('blrt-gallery');
    wp_enqueue_style('open-sans');
    wp_enqueue_style('owlcarousel');
    wp_enqueue_style('blrt-gallery');
    ...
    $html = file_get_contents('gallery.html',true);
    if($a['id'] !== ''){//id exists, do query to database to get content
		...
		if($result === null){
			if (( current_user_can('edit_pages') )) {
	        	$html = "<span class='blrt-gallery-missing'>Blrt gallery '".$a['id']."' not found.</span>";
			} else {
				$html = '';
			}
			return $html;
    ...
    $snipPages = ceil(sizeof($array_url) / $baseItem);
    ...
    $mobilesnipclass = '';
    if ($snipPages <= 1) {
    	$mobilesnipclass = 'disabled';
    }
	$version = time();
	$plugin_dir = plugins_url('', dirname(__FILE__));
    $html = sprintf($html, $a['title'], $a['size'], $a['position'], $a['skin'], $desktop, $mobile, $src, $version, $direction, $no_title, $plugin_dir, $fallBack, $a['mobile'], $snipPages, $mobilesnipclass);
    //$html = $script.$style.$html;
    return $html;
}

add_shortcode( 'blrt-web', 'blrt_web' );
function blrt_web($atts){
	wp_enqueue_style('blrt-web');
	return '<iframe class="blrt-web" src="https://web.blrt.com/" width="100%" height="100%"></iframe>';
}
?>