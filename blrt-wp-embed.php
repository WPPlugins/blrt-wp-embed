...
define('BLRT_WP_EMBED_VERSION', '1.3.7');
define('CDN_URL', '//s3.amazonaws.com/blrt-gallery/'.BLRT_WP_EMBED_VERSION);
...
class BlrtWPEmbed {
    public function __construct() {
        ...
        $this->blrtwpembed_table_gallery_version = '1.1'; //add version number for the table in case we need to update the structure of table later
    }
    
    public function init() {
        wp_register_script('owlcarousel', '//cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.2.1/owl.carousel.min.js', ['jquery'], null, true);
        wp_register_script('ua-parser', '//cdnjs.cloudflare.com/ajax/libs/UAParser.js/0.7.12/ua-parser.min.js', ['jquery'], null, true);
        //wp_register_script('blrt-wp-embed', CDN_URL.'/admin.min.js', ['jquery', 'jquery-ui-sortable'], null, true);
        wp_register_script('blrt-wp-embed', plugins_url('dist/js/admin.min.js', __FILE__), ['jquery', 'jquery-ui-sortable'], BLRT_WP_EMBED_VERSION, true);
        wp_register_script('blrt-gallery', CDN_URL.'/main.min.js', ['jquery', 'owlcarousel', 'ua-parser'], null, true);
        //wp_register_script('blrt-gallery', plugins_url('dist/js/main.min.js', __FILE__), ['jquery', 'owlcarousel', 'ua-parser'], BLRT_WP_EMBED_VERSION, true);
        
        wp_register_style('owlcarousel', '//cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.2.1/assets/owl.carousel.min.css', [], null);
        //wp_register_style('owlcarousel', plugins_url('dist/css/owl.carousel.min.css',  __FILE__), [], '2.2.1');
        //wp_register_style('blrt-wp-embed', CDN_URL.'/admin.min.css', [], null);
        ...
        //wp_register_style('blrt-gallery', plugins_url('dist/css/main.min.css',  __FILE__), [], BLRT_WP_EMBED_VERSION);
        wp_register_style('blrt-web', CDN_URL.'/blrt-web.min.css', [], null);
        //wp_register_style('blrt-web', plugins_url('dist/css/blrt-web.min.css',  __FILE__), [], BLRT_WP_EMBED_VERSION);
        
        add_filter( 'style_loader_tag', array( $this, 'fix_style_tags'), 10, 4);
    ...
    function fix_style_tags($html, $handle, $href, $media) {
        if (in_array($handle, ['blrt-gallery', 'blrt-web', 'blrt-wp-embed', 'owlcarousel'])) {
            return "<link rel='stylesheet' id='".$handle."'  href='".$href."' type='text/css' media='".$media."' crossorigin='anonymous'/>";
        } else {
            return $html;
        }
    }
    ...
    function fallback_assets() {
        ...
        if (wp_script_is( 'jquery', 'enqueued' )) {
            echo "(window.jQuery && jQuery.noConflict()) || document.write('<script src=\"//cdnjs.cloudflare.com/ajax/libs/jquery/2.1.4/jquery.min.js\"><\/script>');\n";
        }
        ...
        if (wp_script_is( 'blrt-gallery', 'enqueued' )) {
            echo "(window.blrt_gallery_js_loaded) || document.write('<script src=\"".plugins_url('dist/js/main.min.js?ver='.BLRT_WP_EMBED_VERSION, __FILE__)."\"><\/script>');\n";
        }
        echo "(function($) {
        ...
        if (wp_style_is( 'owlcarousel', 'enqueued' )) {
            echo "  if (sheet.ownerNode.id == 'owlcarousel') {
                        ...
                        if (rules.length == 0) {
                            //$('<link rel=\"stylesheet\" type=\"text/css\" href=\"//cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.2.1/assets/owl.carousel.min.css\" \/>').appendTo('body');
        ...
        if (wp_style_is( 'blrt-gallery', 'enqueued' )) {
            echo "  if (sheet.ownerNode.id == 'blrt-gallery') {
                        ...
                        if (rules.length == 0) {
                            ...
                            $('<link rel=\"stylesheet\" type=\"text/css\" href=\"".plugins_url('dist/css/main.min.css'.'?ver='.BLRT_WP_EMBED_VERSION,  __FILE__)."\" \/>').appendTo('body');
                        }
                        return;
                    }";
        }
        if (wp_style_is( 'blrt-web', 'enqueued' )) {
            echo "  if (sheet.ownerNode.id == 'blrt-web') {
                        ...
                        if (rules.length == 0) {
                            ...
                            $('<link rel=\"stylesheet\" type=\"text/css\" href=\"".plugins_url('dist/css/blrt-web.min.css'.'?ver='.BLRT_WP_EMBED_VERSION,  __FILE__)."\" \/>').appendTo('body');
                        }
                        return;
                    }";
        }
    ...
    function wpa_add_menu() {
        add_menu_page( 'Blrt', 'Blrt' , 'nosuchcapability', 'blrt', null, '', '25.1');
    ...
    function blrt_admin_bar() {
    	...
    	if (is_array($options) && $options['blrt_web_toolbar_enabled']) {
        	$args = array(
        		'id'    => 'blrt-web',
        		'title' => '<span class="blrt-toolbar ab-icon"></span>',
